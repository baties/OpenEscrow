        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <div className="mt-2 space-y-1">
              <PartyRow
                label="Your ID"
                value={isClient ? deal.clientId : deal.freelancerId}
                action="copy"
              />
              <PartyRow
                label="Your Address"
                value={isClient ? deal.clientAddress : deal.freelancerAddress}
                action="explorer"
                explorerUrl={`${appConfig.chainMeta.explorerUrl}/address/${isClient ? deal.clientAddress : deal.freelancerAddress}`}
              />
              <PartyRow
                label={isClient ? 'Freelancer ID' : 'Client ID'}
                value={isClient ? deal.freelancerId : deal.clientId}
                action="copy"
              />
              <PartyRow
                label={isClient ? 'To Address' : 'From Address'}
                value={isClient ? deal.freelancerAddress : deal.clientAddress}
                action="explorer"
                explorerUrl={`${appConfig.chainMeta.explorerUrl}/address/${isClient ? deal.freelancerAddress : deal.clientAddress}`}
              />
            </div>
            {/* Amount + date row */}
        ...
        <div className="rounded-xl border border-amber-200 bg-amber-50 p-5 shadow-sm space-y-3">
          ...
          {agreeOnchain.step === 'idle' && (
            <button
              type="button"
              onClick={() => {
                if (deal.chainDealId) void agreeOnchain.agree(deal.chainDealId);
              }}
